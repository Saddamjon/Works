...
// Определение таблиц замены (S-boxes)
...
void NESSIEkeysetup(const unsigned char * const key, NESSIEstruct * const structpointer) {
	int N, R, i, pos, r;
	...
	for (r = 0; r <= R; r++) {
		...
		for (i = 0; i < N; i++) {
			int j = i;
			inter[i] = kappa[j--] >> 24; if (j < 0) j = N - 1;
			inter[i] ^= kappa[j--] >> 16 & 0xff; if (j < 0) j = N - 1;
			inter[i] ^= kappa[j--] >> 8 & 0xff; if (j < 0) j = N - 1;
			inter[i] ^= kappa[j] & 0xff;
		}
